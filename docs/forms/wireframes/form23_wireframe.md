...
### 4. List All FORM23 Forms

```http
GET /api/form23/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/form23/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/form23/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_form23 (
    ...
    -- Constraints
    CONSTRAINT ck_form23_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_form23_company (company_id),
    INDEX idx_form23_cin (cin),
    INDEX idx_form23_active (is_active),
    INDEX idx_form23_created (created_on)
);

-- View for joined data
CREATE VIEW vw_form23 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/form23/form23.component.ts
...
export class FORM23Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.form23Form.valid) {
      this.form23Service.createFORM23(this.form23Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/form23_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.form23 import FORM23, FORM23Create, FORM23Update
import logging
...
class FORM23Service:
    ...
    async def get_form23(self, form23_id: int) -> Optional[FORM23]:
        """Get FORM23 by ID"""
        return await self.db.query(FORM23).filter(
            and_(
                FORM23.id == form23_id,
    ...
    async def get_form23s_by_company(
        ...
        """Get all FORM23 forms for a company"""
        return await self.db.query(FORM23).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/form23.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.form23 import FORM23, FORM23Create
...
async def create_form23(
    ...
    """Create new FORM23 form"""
    form23_service = FORM23Service(db)
    return await form23_service.create_form23(
        form23_data, 
        current_user.id
    )

@router.get("/{id}", response_model=FORM23View)
async def get_form23(
    ...
    """Get FORM23 by ID"""
    form23_service = FORM23Service(db)
    form23 = await form23_service.get_form23(id)
    if not form23:
        raise HTTPException(status_code=404, detail="FORM23 not found")
    return form23
```
...
```python
# File: services/forms/app/models/form23.py
...
class FORM23Create(FORM23Base):
    pass

class FORM23Update(FORM23Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class FORM23Response(FORM23Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class FORM23View(FORM23Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → FORM23
   ↓
