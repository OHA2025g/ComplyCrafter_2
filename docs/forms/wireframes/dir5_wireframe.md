...
### 4. List All DIR5 Forms

```http
GET /api/dir5/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/dir5/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/dir5/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_dir5 (
    ...
    -- Constraints
    CONSTRAINT ck_dir5_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_dir5_company (company_id),
    INDEX idx_dir5_cin (cin),
    INDEX idx_dir5_active (is_active),
    INDEX idx_dir5_created (created_on)
);

-- View for joined data
CREATE VIEW vw_dir5 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/dir5/dir5.component.ts
...
export class DIR5Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.dir5Form.valid) {
      this.dir5Service.createDIR5(this.dir5Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/dir5_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.dir5 import DIR5, DIR5Create, DIR5Update
import logging
...
class DIR5Service:
    ...
    async def get_dir5(self, dir5_id: int) -> Optional[DIR5]:
        """Get DIR5 by ID"""
        return await self.db.query(DIR5).filter(
            and_(
                DIR5.id == dir5_id,
                DIR5.is_active == True
            )
    ...
    async def get_dir5s_by_company(
        ...
        """Get all DIR5 forms for a company"""
        return await self.db.query(DIR5).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/dir5.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.dir5 import DIR5, DIR5Create
...
async def create_dir5(
    ...
    """Create new DIR5 form"""
    dir5_service = DIR5Service(db)
    return await dir5_service.create_dir5(
        dir5_data, 
        current_user.id
    )

@router.get("/{id}", response_model=DIR5View)
async def get_dir5(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get DIR5 by ID"""
    dir5_service = DIR5Service(db)
    dir5 = await dir5_service.get_dir5(id)
    if not dir5:
        raise HTTPException(status_code=404, detail="DIR5 not found")
    return dir5
```
...
```python
# File: services/forms/app/models/dir5.py
...
class DIR5Create(DIR5Base):
    pass

class DIR5Update(DIR5Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class DIR5Response(DIR5Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class DIR5View(DIR5Response):
