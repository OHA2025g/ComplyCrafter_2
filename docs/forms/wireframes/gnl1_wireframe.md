# GNL1: General Purpose Form - 1

**Form Code:** GNL1  
**Form Name:** General Purpose Form - 1  
...
### 4. List All GNL1 Forms

```http
GET /api/gnl1/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/gnl1/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/gnl1/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_gnl1 (
    ...
    -- Constraints
    CONSTRAINT ck_gnl1_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_gnl1_company (company_id),
    INDEX idx_gnl1_cin (cin),
    INDEX idx_gnl1_active (is_active),
    INDEX idx_gnl1_created (created_on)
);

-- View for joined data
CREATE VIEW vw_gnl1 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/gnl1/gnl1.component.ts
...
export class GNL1Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.gnl1Form.valid) {
      this.gnl1Service.createGNL1(this.gnl1Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/gnl1_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.gnl1 import GNL1, GNL1Create, GNL1Update
import logging
...
class GNL1Service:
    ...
    async def get_gnl1(self, gnl1_id: int) -> Optional[GNL1]:
        """Get GNL1 by ID"""
        return await self.db.query(GNL1).filter(
            and_(
                GNL1.id == gnl1_id,
                GNL1.is_active == True
            )
    ...
    async def get_gnl1s_by_company(
        ...
        """Get all GNL1 forms for a company"""
        return await self.db.query(GNL1).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/gnl1.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.gnl1 import GNL1, GNL1Create
...
async def create_gnl1(
    ...
    """Create new GNL1 form"""
    gnl1_service = GNL1Service(db)
    return await gnl1_service.create_gnl1(
        gnl1_data, 
        current_user.id
    )

@router.get("/{id}", response_model=GNL1View)
async def get_gnl1(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get GNL1 by ID"""
    gnl1_service = GNL1Service(db)
    gnl1 = await gnl1_service.get_gnl1(id)
    if not gnl1:
        raise HTTPException(status_code=404, detail="GNL1 not found")
    return gnl1
```
...
```python
# File: services/forms/app/models/gnl1.py
...
class GNL1Create(GNL1Base):
    pass

class GNL1Update(GNL1Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class GNL1Response(GNL1Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class GNL1View(GNL1Response):
