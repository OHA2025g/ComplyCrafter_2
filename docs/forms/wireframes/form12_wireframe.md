...
### 4. List All FORM12 Forms

```http
GET /api/form12/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/form12/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/form12/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_form12 (
    ...
    -- Constraints
    CONSTRAINT ck_form12_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_form12_company (company_id),
    INDEX idx_form12_cin (cin),
    INDEX idx_form12_active (is_active),
    INDEX idx_form12_created (created_on)
);

-- View for joined data
CREATE VIEW vw_form12 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/form12/form12.component.ts
...
export class FORM12Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.form12Form.valid) {
      this.form12Service.createFORM12(this.form12Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/form12_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.form12 import FORM12, FORM12Create, FORM12Update
import logging
...
class FORM12Service:
    ...
    async def get_form12(self, form12_id: int) -> Optional[FORM12]:
        """Get FORM12 by ID"""
        return await self.db.query(FORM12).filter(
            and_(
                FORM12.id == form12_id,
    ...
    async def get_form12s_by_company(
        ...
        """Get all FORM12 forms for a company"""
        return await self.db.query(FORM12).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/form12.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.form12 import FORM12, FORM12Create
...
async def create_form12(
    ...
    """Create new FORM12 form"""
    form12_service = FORM12Service(db)
    return await form12_service.create_form12(
        form12_data, 
        current_user.id
    )

@router.get("/{id}", response_model=FORM12View)
async def get_form12(
    ...
    """Get FORM12 by ID"""
    form12_service = FORM12Service(db)
    form12 = await form12_service.get_form12(id)
    if not form12:
        raise HTTPException(status_code=404, detail="FORM12 not found")
    return form12
```
...
```python
# File: services/forms/app/models/form12.py
...
class FORM12Create(FORM12Base):
    pass

class FORM12Update(FORM12Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class FORM12Response(FORM12Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class FORM12View(FORM12Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → FORM12
   ↓
