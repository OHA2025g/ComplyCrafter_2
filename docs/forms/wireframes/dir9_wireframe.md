...
### 4. List All DIR9 Forms

```http
GET /api/dir9/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/dir9/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/dir9/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_dir9 (
    ...
    -- Constraints
    CONSTRAINT ck_dir9_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_dir9_company (company_id),
    INDEX idx_dir9_cin (cin),
    INDEX idx_dir9_active (is_active),
    INDEX idx_dir9_created (created_on)
);

-- View for joined data
CREATE VIEW vw_dir9 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/dir9/dir9.component.ts
...
export class DIR9Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.dir9Form.valid) {
      this.dir9Service.createDIR9(this.dir9Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/dir9_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.dir9 import DIR9, DIR9Create, DIR9Update
import logging
...
class DIR9Service:
    ...
    async def get_dir9(self, dir9_id: int) -> Optional[DIR9]:
        """Get DIR9 by ID"""
        return await self.db.query(DIR9).filter(
            and_(
                DIR9.id == dir9_id,
                DIR9.is_active == True
            )
    ...
    async def get_dir9s_by_company(
        ...
        """Get all DIR9 forms for a company"""
        return await self.db.query(DIR9).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/dir9.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.dir9 import DIR9, DIR9Create
...
async def create_dir9(
    ...
    """Create new DIR9 form"""
    dir9_service = DIR9Service(db)
    return await dir9_service.create_dir9(
        dir9_data, 
        current_user.id
    )

@router.get("/{id}", response_model=DIR9View)
async def get_dir9(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get DIR9 by ID"""
    dir9_service = DIR9Service(db)
    dir9 = await dir9_service.get_dir9(id)
    if not dir9:
        raise HTTPException(status_code=404, detail="DIR9 not found")
    return dir9
```
...
```python
# File: services/forms/app/models/dir9.py
...
class DIR9Create(DIR9Base):
    pass

class DIR9Update(DIR9Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class DIR9Response(DIR9Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class DIR9View(DIR9Response):
