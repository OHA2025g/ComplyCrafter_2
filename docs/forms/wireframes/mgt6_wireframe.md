# MGT6: Return of Compliance Certificate

**Form Code:** MGT6  
**Form Name:** Return of Compliance Certificate  
**Regulation:** Companies Act, 2013 - Section 204  
**Filing Timeline:** Within 60 days of financial year end  
**Field Count:** 65  
**Status:** ✅ Migrated
...
┌─────────────────────────────────────────────────────────────────────────────┐
│                       Return of Compliance Certificate                      │
└─────────────────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────────────────┐
│ [Details]  [Compliance]  [Certificate]  [Attachments]                       │
└─────────────────────────────────────────────────────────────────────────────┘
...
### 4. List All MGT6 Forms

```http
GET /api/mgt6/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/mgt6/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/mgt6/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_mgt6 (
    ...
    -- Constraints
    CONSTRAINT ck_mgt6_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_mgt6_company (company_id),
    INDEX idx_mgt6_cin (cin),
    INDEX idx_mgt6_active (is_active),
    INDEX idx_mgt6_created (created_on)
);

-- View for joined data
CREATE VIEW vw_mgt6 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/mgt6/mgt6.component.ts
...
export class MGT6Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.mgt6Form.valid) {
      this.mgt6Service.createMGT6(this.mgt6Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/mgt6_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.mgt6 import MGT6, MGT6Create, MGT6Update
import logging
...
class MGT6Service:
    ...
    async def get_mgt6(self, mgt6_id: int) -> Optional[MGT6]:
        """Get MGT6 by ID"""
        return await self.db.query(MGT6).filter(
            and_(
                MGT6.id == mgt6_id,
                MGT6.is_active == True
            )
    ...
    async def get_mgt6s_by_company(
        ...
        """Get all MGT6 forms for a company"""
        return await self.db.query(MGT6).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/mgt6.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.mgt6 import MGT6, MGT6Create
...
async def create_mgt6(
    ...
    """Create new MGT6 form"""
    mgt6_service = MGT6Service(db)
    return await mgt6_service.create_mgt6(
        mgt6_data, 
        current_user.id
    )

@router.get("/{id}", response_model=MGT6View)
async def get_mgt6(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get MGT6 by ID"""
    mgt6_service = MGT6Service(db)
    mgt6 = await mgt6_service.get_mgt6(id)
    if not mgt6:
        raise HTTPException(status_code=404, detail="MGT6 not found")
    return mgt6
```
...
```python
# File: services/forms/app/models/mgt6.py
...
class MGT6Create(MGT6Base):
    pass

class MGT6Update(MGT6Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class MGT6Response(MGT6Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class MGT6View(MGT6Response):
