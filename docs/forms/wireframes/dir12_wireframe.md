...
### 4. List All DIR12 Forms

```http
GET /api/dir12/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/dir12/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/dir12/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_dir12 (
    ...
    -- Constraints
    CONSTRAINT ck_dir12_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_dir12_company (company_id),
    INDEX idx_dir12_cin (cin),
    INDEX idx_dir12_active (is_active),
    INDEX idx_dir12_created (created_on)
);

-- View for joined data
CREATE VIEW vw_dir12 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/dir12/dir12.component.ts
...
export class DIR12Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.dir12Form.valid) {
      this.dir12Service.createDIR12(this.dir12Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/dir12_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.dir12 import DIR12, DIR12Create, DIR12Update
import logging
...
class DIR12Service:
    ...
    async def get_dir12(self, dir12_id: int) -> Optional[DIR12]:
        """Get DIR12 by ID"""
        return await self.db.query(DIR12).filter(
            and_(
                DIR12.id == dir12_id,
    ...
    async def get_dir12s_by_company(
        ...
        """Get all DIR12 forms for a company"""
        return await self.db.query(DIR12).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/dir12.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.dir12 import DIR12, DIR12Create
...
async def create_dir12(
    ...
    """Create new DIR12 form"""
    dir12_service = DIR12Service(db)
    return await dir12_service.create_dir12(
        dir12_data, 
        current_user.id
    )

@router.get("/{id}", response_model=DIR12View)
async def get_dir12(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get DIR12 by ID"""
    dir12_service = DIR12Service(db)
    dir12 = await dir12_service.get_dir12(id)
    if not dir12:
        raise HTTPException(status_code=404, detail="DIR12 not found")
    return dir12
```
...
```python
# File: services/forms/app/models/dir12.py
...
class DIR12Create(DIR12Base):
    pass

class DIR12Update(DIR12Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class DIR12Response(DIR12Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class DIR12View(DIR12Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → DIR12
   ↓
