...
### 4. List All PAS2 Forms

```http
GET /api/pas2/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/pas2/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/pas2/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_pas2 (
    ...
    -- Constraints
    CONSTRAINT ck_pas2_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_pas2_company (company_id),
    INDEX idx_pas2_cin (cin),
    INDEX idx_pas2_active (is_active),
    INDEX idx_pas2_created (created_on)
);

-- View for joined data
CREATE VIEW vw_pas2 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/pas2/pas2.component.ts
...
export class PAS2Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.pas2Form.valid) {
      this.pas2Service.createPAS2(this.pas2Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/pas2_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.pas2 import PAS2, PAS2Create, PAS2Update
import logging
...
class PAS2Service:
    ...
    async def get_pas2(self, pas2_id: int) -> Optional[PAS2]:
        """Get PAS2 by ID"""
        return await self.db.query(PAS2).filter(
            and_(
                PAS2.id == pas2_id,
                PAS2.is_active == True
            )
    ...
    async def get_pas2s_by_company(
        ...
        """Get all PAS2 forms for a company"""
        return await self.db.query(PAS2).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/pas2.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.pas2 import PAS2, PAS2Create
...
async def create_pas2(
    ...
    """Create new PAS2 form"""
    pas2_service = PAS2Service(db)
    return await pas2_service.create_pas2(
        pas2_data, 
        current_user.id
    )

@router.get("/{id}", response_model=PAS2View)
async def get_pas2(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get PAS2 by ID"""
    pas2_service = PAS2Service(db)
    pas2 = await pas2_service.get_pas2(id)
    if not pas2:
        raise HTTPException(status_code=404, detail="PAS2 not found")
    return pas2
```
...
```python
# File: services/forms/app/models/pas2.py
...
class PAS2Create(PAS2Base):
    pass

class PAS2Update(PAS2Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class PAS2Response(PAS2Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class PAS2View(PAS2Response):
