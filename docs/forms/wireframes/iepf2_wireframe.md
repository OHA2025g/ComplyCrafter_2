# IEPF2: Statement of Unclaimed and Unpaid Amounts

**Form Code:** IEPF2  
**Form Name:** Statement of Unclaimed and Unpaid Amounts  
**Regulation:** IEPF Rules, 2016  
**Filing Timeline:** Annually by 30th September  
**Field Count:** 75  
**Status:** ✅ Migrated
...
┌─────────────────────────────────────────────────────────────────────────────┐
│ [Details]  [Amount Breakup]  [Attachments]                                  │
└─────────────────────────────────────────────────────────────────────────────┘
...
### 4. List All IEPF2 Forms

```http
GET /api/iepf2/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/iepf2/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/iepf2/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_iepf2 (
    ...
    -- Constraints
    CONSTRAINT ck_iepf2_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_iepf2_company (company_id),
    INDEX idx_iepf2_cin (cin),
    INDEX idx_iepf2_active (is_active),
    INDEX idx_iepf2_created (created_on)
);

-- View for joined data
CREATE VIEW vw_iepf2 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/iepf2/iepf2.component.ts
...
export class IEPF2Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.iepf2Form.valid) {
      this.iepf2Service.createIEPF2(this.iepf2Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/iepf2_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.iepf2 import IEPF2, IEPF2Create, IEPF2Update
import logging
...
class IEPF2Service:
    ...
    async def get_iepf2(self, iepf2_id: int) -> Optional[IEPF2]:
        """Get IEPF2 by ID"""
        return await self.db.query(IEPF2).filter(
            and_(
                IEPF2.id == iepf2_id,
    ...
    async def get_iepf2s_by_company(
        ...
        """Get all IEPF2 forms for a company"""
        return await self.db.query(IEPF2).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/iepf2.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.iepf2 import IEPF2, IEPF2Create
...
async def create_iepf2(
    ...
    """Create new IEPF2 form"""
    iepf2_service = IEPF2Service(db)
    return await iepf2_service.create_iepf2(
        iepf2_data, 
        current_user.id
    )

@router.get("/{id}", response_model=IEPF2View)
async def get_iepf2(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get IEPF2 by ID"""
    iepf2_service = IEPF2Service(db)
    iepf2 = await iepf2_service.get_iepf2(id)
    if not iepf2:
        raise HTTPException(status_code=404, detail="IEPF2 not found")
    return iepf2
```
...
```python
# File: services/forms/app/models/iepf2.py
...
class IEPF2Create(IEPF2Base):
    pass

class IEPF2Update(IEPF2Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class IEPF2Response(IEPF2Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class IEPF2View(IEPF2Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → IEPF2
   ↓
