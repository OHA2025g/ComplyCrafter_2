...
### 4. List All DIR11 Forms

```http
GET /api/dir11/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/dir11/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/dir11/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_dir11 (
    ...
    -- Constraints
    CONSTRAINT ck_dir11_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_dir11_company (company_id),
    INDEX idx_dir11_cin (cin),
    INDEX idx_dir11_active (is_active),
    INDEX idx_dir11_created (created_on)
);

-- View for joined data
CREATE VIEW vw_dir11 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/dir11/dir11.component.ts
...
export class DIR11Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.dir11Form.valid) {
      this.dir11Service.createDIR11(this.dir11Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/dir11_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.dir11 import DIR11, DIR11Create, DIR11Update
import logging
...
class DIR11Service:
    ...
    async def get_dir11(self, dir11_id: int) -> Optional[DIR11]:
        """Get DIR11 by ID"""
        return await self.db.query(DIR11).filter(
            and_(
                DIR11.id == dir11_id,
    ...
    async def get_dir11s_by_company(
        ...
        """Get all DIR11 forms for a company"""
        return await self.db.query(DIR11).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/dir11.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.dir11 import DIR11, DIR11Create
...
async def create_dir11(
    ...
    """Create new DIR11 form"""
    dir11_service = DIR11Service(db)
    return await dir11_service.create_dir11(
        dir11_data, 
        current_user.id
    )

@router.get("/{id}", response_model=DIR11View)
async def get_dir11(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get DIR11 by ID"""
    dir11_service = DIR11Service(db)
    dir11 = await dir11_service.get_dir11(id)
    if not dir11:
        raise HTTPException(status_code=404, detail="DIR11 not found")
    return dir11
```
...
```python
# File: services/forms/app/models/dir11.py
...
class DIR11Create(DIR11Base):
    pass

class DIR11Update(DIR11Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class DIR11Response(DIR11Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class DIR11View(DIR11Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → DIR11
   ↓
