...
### 4. List All MGT14 Forms

```http
GET /api/mgt14/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/mgt14/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/mgt14/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_mgt14 (
    ...
    -- Constraints
    CONSTRAINT ck_mgt14_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_mgt14_company (company_id),
    INDEX idx_mgt14_cin (cin),
    INDEX idx_mgt14_active (is_active),
    INDEX idx_mgt14_created (created_on)
);

-- View for joined data
CREATE VIEW vw_mgt14 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/mgt14/mgt14.component.ts
...
export class MGT14Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.mgt14Form.valid) {
      this.mgt14Service.createMGT14(this.mgt14Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/mgt14_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.mgt14 import MGT14, MGT14Create, MGT14Update
import logging
...
class MGT14Service:
    ...
    async def get_mgt14(self, mgt14_id: int) -> Optional[MGT14]:
        """Get MGT14 by ID"""
        return await self.db.query(MGT14).filter(
            and_(
                MGT14.id == mgt14_id,
    ...
    async def get_mgt14s_by_company(
        ...
        """Get all MGT14 forms for a company"""
        return await self.db.query(MGT14).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/mgt14.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.mgt14 import MGT14, MGT14Create
...
async def create_mgt14(
    ...
    """Create new MGT14 form"""
    mgt14_service = MGT14Service(db)
    return await mgt14_service.create_mgt14(
        mgt14_data, 
        current_user.id
    )

@router.get("/{id}", response_model=MGT14View)
async def get_mgt14(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get MGT14 by ID"""
    mgt14_service = MGT14Service(db)
    mgt14 = await mgt14_service.get_mgt14(id)
    if not mgt14:
        raise HTTPException(status_code=404, detail="MGT14 not found")
    return mgt14
```
...
```python
# File: services/forms/app/models/mgt14.py
...
class MGT14Create(MGT14Base):
    pass

class MGT14Update(MGT14Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class MGT14Response(MGT14Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class MGT14View(MGT14Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → MGT14
   ↓
