...
### 4. List All CHG9 Forms

```http
GET /api/chg9/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/chg9/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/chg9/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_chg9 (
    ...
    -- Constraints
    CONSTRAINT ck_chg9_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_chg9_company (company_id),
    INDEX idx_chg9_cin (cin),
    INDEX idx_chg9_active (is_active),
    INDEX idx_chg9_created (created_on)
);

-- View for joined data
CREATE VIEW vw_chg9 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/chg9/chg9.component.ts
...
export class CHG9Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.chg9Form.valid) {
      this.chg9Service.createCHG9(this.chg9Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/chg9_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.chg9 import CHG9, CHG9Create, CHG9Update
import logging
...
class CHG9Service:
    ...
    async def get_chg9(self, chg9_id: int) -> Optional[CHG9]:
        """Get CHG9 by ID"""
        return await self.db.query(CHG9).filter(
            and_(
                CHG9.id == chg9_id,
                CHG9.is_active == True
            )
    ...
    async def get_chg9s_by_company(
        ...
        """Get all CHG9 forms for a company"""
        return await self.db.query(CHG9).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/chg9.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.chg9 import CHG9, CHG9Create
...
async def create_chg9(
    ...
    """Create new CHG9 form"""
    chg9_service = CHG9Service(db)
    return await chg9_service.create_chg9(
        chg9_data, 
        current_user.id
    )

@router.get("/{id}", response_model=CHG9View)
async def get_chg9(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get CHG9 by ID"""
    chg9_service = CHG9Service(db)
    chg9 = await chg9_service.get_chg9(id)
    if not chg9:
        raise HTTPException(status_code=404, detail="CHG9 not found")
    return chg9
```
...
```python
# File: services/forms/app/models/chg9.py
...
class CHG9Create(CHG9Base):
    pass

class CHG9Update(CHG9Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class CHG9Response(CHG9Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class CHG9View(CHG9Response):
