...
### 4. List All FORM15 Forms

```http
GET /api/form15/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/form15/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/form15/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_form15 (
    ...
    -- Constraints
    CONSTRAINT ck_form15_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_form15_company (company_id),
    INDEX idx_form15_cin (cin),
    INDEX idx_form15_active (is_active),
    INDEX idx_form15_created (created_on)
);

-- View for joined data
CREATE VIEW vw_form15 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/form15/form15.component.ts
...
export class FORM15Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.form15Form.valid) {
      this.form15Service.createFORM15(this.form15Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/form15_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.form15 import FORM15, FORM15Create, FORM15Update
import logging
...
class FORM15Service:
    ...
    async def get_form15(self, form15_id: int) -> Optional[FORM15]:
        """Get FORM15 by ID"""
        return await self.db.query(FORM15).filter(
            and_(
                FORM15.id == form15_id,
    ...
    async def get_form15s_by_company(
        ...
        """Get all FORM15 forms for a company"""
        return await self.db.query(FORM15).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/form15.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.form15 import FORM15, FORM15Create
...
async def create_form15(
    ...
    """Create new FORM15 form"""
    form15_service = FORM15Service(db)
    return await form15_service.create_form15(
        form15_data, 
        current_user.id
    )

@router.get("/{id}", response_model=FORM15View)
async def get_form15(
    ...
    """Get FORM15 by ID"""
    form15_service = FORM15Service(db)
    form15 = await form15_service.get_form15(id)
    if not form15:
        raise HTTPException(status_code=404, detail="FORM15 not found")
    return form15
```
...
```python
# File: services/forms/app/models/form15.py
...
class FORM15Create(FORM15Base):
    pass

class FORM15Update(FORM15Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class FORM15Response(FORM15Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class FORM15View(FORM15Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → FORM15
   ↓
