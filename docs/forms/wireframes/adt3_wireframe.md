# ADT3: Notice of Resignation of Auditor
...
┌─────────────────────────────────────────────────────────────────────────────┐
│ [Details]  [Reasons]  [DSC Details]  [Attachments]                          │
└─────────────────────────────────────────────────────────────────────────────┘
...
### 4. List All ADT3 Forms

```http
GET /api/adt3/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/adt3/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/adt3/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_adt3 (
    ...
    -- Constraints
    CONSTRAINT ck_adt3_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_adt3_company (company_id),
    INDEX idx_adt3_cin (cin),
    INDEX idx_adt3_active (is_active),
    INDEX idx_adt3_created (created_on)
);

-- View for joined data
CREATE VIEW vw_adt3 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/adt3/adt3.component.ts

import { Component, OnInit } from '@angular/core';
import { FormBuilder, FormGroup, Validators } from '@angular/forms';
import { ADT3Service } from './adt3.service';

@Component({
  selector: 'app-adt3',
  templateUrl: './adt3.component.html',
  styleUrls: ['./adt3.component.scss']
})
export class ADT3Component implements OnInit {
  adt3Form!: FormGroup;
  isEditMode = false;
  activeTab = 'details';
  ...
  constructor(
    ...
    private adt3Service: ADT3Service
  ) {
    this.initializeForm();
  }
  ...
  onSubmit(): void {
    if (this.adt3Form.valid) {
      this.adt3Service.createADT3(this.adt3Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/adt3_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.adt3 import ADT3, ADT3Create, ADT3Update
import logging
...
class ADT3Service:
    ...
    async def create_adt3(
        self, 
        adt3_data: ADT3Create, 
        user_id: int
    ) -> ADT3:
        """Create a new ADT3 record"""
        try:
            adt3 = ADT3(
                **adt3_data.dict(),
                created_by=user_id,
            ...
            await self.db.refresh(adt3)
            logger.info(f"Created ADT3 with ID: {adt3.id}")
            return adt3
        except Exception as e:
            await self.db.rollback()
            logger.error(f"Error creating ADT3: {str(e)}")
            raise
    
    async def get_adt3(self, adt3_id: int) -> Optional[ADT3]:
        """Get ADT3 by ID"""
        return await self.db.query(ADT3).filter(
            and_(
                ADT3.id == adt3_id,
                ADT3.is_active == True
            )
    ...
    async def get_adt3s_by_company(
        self, 
        company_id: int
    ) -> List[ADT3]:
        """Get all ADT3 forms for a company"""
        return await self.db.query(ADT3).filter(
            and_(
                ADT3.company_id == company_id,
                ADT3.is_active == True
            )
        ).all()
```
...
```python
# File: services/forms/app/api/routes/adt3.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.adt3 import ADT3, ADT3Create
from ...services.adt3_service import ADT3Service

router = APIRouter(prefix="/adt3", tags=["adt3"])

@router.post("/", response_model=ADT3View, status_code=status.HTTP_201_CREATED)
async def create_adt3(
    adt3_data: ADT3Create,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Create new ADT3 form"""
    adt3_service = ADT3Service(db)
    return await adt3_service.create_adt3(
        adt3_data, 
        current_user.id
    )

@router.get("/{id}", response_model=ADT3View)
async def get_adt3(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get ADT3 by ID"""
    adt3_service = ADT3Service(db)
    adt3 = await adt3_service.get_adt3(id)
    if not adt3:
        raise HTTPException(status_code=404, detail="ADT3 not found")
    return adt3
```
...
```python
# File: services/forms/app/models/adt3.py
...
class ADT3Create(ADT3Base):
    pass

class ADT3Update(ADT3Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class ADT3Response(ADT3Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class ADT3View(ADT3Response):
