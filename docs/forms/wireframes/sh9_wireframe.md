...
### 4. List All SH9 Forms

```http
GET /api/sh9/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/sh9/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/sh9/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_sh9 (
    ...
    -- Constraints
    CONSTRAINT ck_sh9_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_sh9_company (company_id),
    INDEX idx_sh9_cin (cin),
    INDEX idx_sh9_active (is_active),
    INDEX idx_sh9_created (created_on)
);

-- View for joined data
CREATE VIEW vw_sh9 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/sh9/sh9.component.ts
...
export class SH9Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.sh9Form.valid) {
      this.sh9Service.createSH9(this.sh9Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/sh9_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.sh9 import SH9, SH9Create, SH9Update
import logging
...
class SH9Service:
    ...
    async def get_sh9(self, sh9_id: int) -> Optional[SH9]:
        """Get SH9 by ID"""
        return await self.db.query(SH9).filter(
            and_(
                SH9.id == sh9_id,
                SH9.is_active == True
            )
    ...
    async def get_sh9s_by_company(
        ...
        """Get all SH9 forms for a company"""
        return await self.db.query(SH9).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/sh9.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.sh9 import SH9, SH9Create
...
async def create_sh9(
    ...
    return await sh9_service.create_sh9(
        sh9_data, 
        current_user.id
    )

@router.get("/{id}", response_model=SH9View)
async def get_sh9(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get SH9 by ID"""
    sh9_service = SH9Service(db)
    sh9 = await sh9_service.get_sh9(id)
    if not sh9:
        raise HTTPException(status_code=404, detail="SH9 not found")
    return sh9
```
...
```python
# File: services/forms/app/models/sh9.py
...
class SH9Create(SH9Base):
    pass

class SH9Update(SH9Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class SH9Response(SH9Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class SH9View(SH9Response):
