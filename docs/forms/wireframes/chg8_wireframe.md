...
### 4. List All CHG8 Forms

```http
GET /api/chg8/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/chg8/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/chg8/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_chg8 (
    ...
    -- Constraints
    CONSTRAINT ck_chg8_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_chg8_company (company_id),
    INDEX idx_chg8_cin (cin),
    INDEX idx_chg8_active (is_active),
    INDEX idx_chg8_created (created_on)
);

-- View for joined data
CREATE VIEW vw_chg8 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/chg8/chg8.component.ts
...
export class CHG8Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.chg8Form.valid) {
      this.chg8Service.createCHG8(this.chg8Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/chg8_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.chg8 import CHG8, CHG8Create, CHG8Update
import logging
...
class CHG8Service:
    ...
    async def get_chg8(self, chg8_id: int) -> Optional[CHG8]:
        """Get CHG8 by ID"""
        return await self.db.query(CHG8).filter(
            and_(
                CHG8.id == chg8_id,
                CHG8.is_active == True
            )
    ...
    async def get_chg8s_by_company(
        ...
        """Get all CHG8 forms for a company"""
        return await self.db.query(CHG8).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/chg8.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.chg8 import CHG8, CHG8Create
...
async def create_chg8(
    ...
    """Create new CHG8 form"""
    chg8_service = CHG8Service(db)
    return await chg8_service.create_chg8(
        chg8_data, 
        current_user.id
    )

@router.get("/{id}", response_model=CHG8View)
async def get_chg8(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get CHG8 by ID"""
    chg8_service = CHG8Service(db)
    chg8 = await chg8_service.get_chg8(id)
    if not chg8:
        raise HTTPException(status_code=404, detail="CHG8 not found")
    return chg8
```
...
```python
# File: services/forms/app/models/chg8.py
...
class CHG8Create(CHG8Base):
    pass

class CHG8Update(CHG8Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class CHG8Response(CHG8Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class CHG8View(CHG8Response):
