...
### 4. List All INC23 Forms

```http
GET /api/inc23/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/inc23/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/inc23/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_inc23 (
    ...
    -- Constraints
    CONSTRAINT ck_inc23_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_inc23_company (company_id),
    INDEX idx_inc23_cin (cin),
    INDEX idx_inc23_active (is_active),
    INDEX idx_inc23_created (created_on)
);

-- View for joined data
CREATE VIEW vw_inc23 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/inc23/inc23.component.ts
...
export class INC23Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.inc23Form.valid) {
      this.inc23Service.createINC23(this.inc23Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/inc23_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.inc23 import INC23, INC23Create, INC23Update
import logging
...
class INC23Service:
    ...
    async def get_inc23(self, inc23_id: int) -> Optional[INC23]:
        """Get INC23 by ID"""
        return await self.db.query(INC23).filter(
            and_(
                INC23.id == inc23_id,
    ...
    async def get_inc23s_by_company(
        ...
        """Get all INC23 forms for a company"""
        return await self.db.query(INC23).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/inc23.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.inc23 import INC23, INC23Create
...
async def create_inc23(
    ...
    """Create new INC23 form"""
    inc23_service = INC23Service(db)
    return await inc23_service.create_inc23(
        inc23_data, 
        current_user.id
    )

@router.get("/{id}", response_model=INC23View)
async def get_inc23(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get INC23 by ID"""
    inc23_service = INC23Service(db)
    inc23 = await inc23_service.get_inc23(id)
    if not inc23:
        raise HTTPException(status_code=404, detail="INC23 not found")
    return inc23
```
...
```python
# File: services/forms/app/models/inc23.py
...
class INC23Create(INC23Base):
    pass

class INC23Update(INC23Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class INC23Response(INC23Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class INC23View(INC23Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → INC23
   ↓
