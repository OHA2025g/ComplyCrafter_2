...
### 4. List All INC28 Forms

```http
GET /api/inc28/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/inc28/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/inc28/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_inc28 (
    ...
    -- Constraints
    CONSTRAINT ck_inc28_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_inc28_company (company_id),
    INDEX idx_inc28_cin (cin),
    INDEX idx_inc28_active (is_active),
    INDEX idx_inc28_created (created_on)
);

-- View for joined data
CREATE VIEW vw_inc28 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/inc28/inc28.component.ts
...
export class INC28Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.inc28Form.valid) {
      this.inc28Service.createINC28(this.inc28Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/inc28_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.inc28 import INC28, INC28Create, INC28Update
import logging
...
class INC28Service:
    ...
    async def get_inc28(self, inc28_id: int) -> Optional[INC28]:
        """Get INC28 by ID"""
        return await self.db.query(INC28).filter(
            and_(
                INC28.id == inc28_id,
    ...
    async def get_inc28s_by_company(
        ...
        """Get all INC28 forms for a company"""
        return await self.db.query(INC28).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/inc28.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.inc28 import INC28, INC28Create
...
async def create_inc28(
    ...
    """Create new INC28 form"""
    inc28_service = INC28Service(db)
    return await inc28_service.create_inc28(
        inc28_data, 
        current_user.id
    )

@router.get("/{id}", response_model=INC28View)
async def get_inc28(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get INC28 by ID"""
    inc28_service = INC28Service(db)
    inc28 = await inc28_service.get_inc28(id)
    if not inc28:
        raise HTTPException(status_code=404, detail="INC28 not found")
    return inc28
```
...
```python
# File: services/forms/app/models/inc28.py
...
class INC28Create(INC28Base):
    pass

class INC28Update(INC28Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class INC28Response(INC28Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class INC28View(INC28Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → INC28
   ↓
