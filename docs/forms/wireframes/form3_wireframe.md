...
### 4. List All FORM3 Forms

```http
GET /api/form3/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/form3/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/form3/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_form3 (
    ...
    -- Constraints
    CONSTRAINT ck_form3_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_form3_company (company_id),
    INDEX idx_form3_cin (cin),
    INDEX idx_form3_active (is_active),
    INDEX idx_form3_created (created_on)
);

-- View for joined data
CREATE VIEW vw_form3 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/form3/form3.component.ts
...
export class FORM3Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.form3Form.valid) {
      this.form3Service.createFORM3(this.form3Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/form3_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.form3 import FORM3, FORM3Create, FORM3Update
import logging
...
class FORM3Service:
    ...
    async def get_form3(self, form3_id: int) -> Optional[FORM3]:
        """Get FORM3 by ID"""
        return await self.db.query(FORM3).filter(
            and_(
                FORM3.id == form3_id,
    ...
    async def get_form3s_by_company(
        ...
        """Get all FORM3 forms for a company"""
        return await self.db.query(FORM3).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/form3.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.form3 import FORM3, FORM3Create
...
async def create_form3(
    ...
    """Create new FORM3 form"""
    form3_service = FORM3Service(db)
    return await form3_service.create_form3(
        form3_data, 
        current_user.id
    )

@router.get("/{id}", response_model=FORM3View)
async def get_form3(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get FORM3 by ID"""
    form3_service = FORM3Service(db)
    form3 = await form3_service.get_form3(id)
    if not form3:
        raise HTTPException(status_code=404, detail="FORM3 not found")
    return form3
```
...
```python
# File: services/forms/app/models/form3.py
...
class FORM3Create(FORM3Base):
    pass

class FORM3Update(FORM3Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class FORM3Response(FORM3Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class FORM3View(FORM3Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → FORM3
   ↓
