# BOARDREPORT: Form BOARDREPORT

**Form Code:** BOARDREPORT  
**Form Name:** N/A  
**Regulation:** N/A  
...
### 1. Create BOARDREPORT

```http
POST /api/boardreport/
...
### 2. Get BOARDREPORT by ID

```http
GET /api/boardreport/{id}
Authorization: Bearer {token}
...
### 3. Update BOARDREPORT

```http
PUT /api/boardreport/{id}
Content-Type: application/json
Authorization: Bearer {token}
...
### 4. List All BOARDREPORT Forms

```http
GET /api/boardreport/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/boardreport/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/boardreport/{id}/status/{status}
Authorization: Bearer {token}
...
### 7. Delete BOARDREPORT

```http
DELETE /api/boardreport/{id}
Authorization: Bearer {token}
...
### Table: `tbl_boardreport`

```sql
CREATE TABLE tbl_boardreport (
    -- Primary Key
    ...
    -- Constraints
    CONSTRAINT ck_boardreport_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_boardreport_company (company_id),
    INDEX idx_boardreport_cin (cin),
    INDEX idx_boardreport_active (is_active),
    INDEX idx_boardreport_created (created_on)
);

-- View for joined data
CREATE VIEW vw_boardreport AS
SELECT 
    f.*,
    u.name as ref_user_name,
    c.name as company_name,
    c.email as company_email
FROM tbl_boardreport f
...
```typescript
// File: frontend/apps/portal/src/app/forms/boardreport/boardreport.component.ts
...
export class BOARDREPORTComponent implements OnInit {
  ...
  onSubmit(): void {
    if (this.boardreportForm.valid) {
      this.boardreportService.createBOARDREPORT(this.boardreportForm.value)
        .subscribe({
...
```python
# File: services/forms/app/services/boardreport_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.boardreport import BOARDREPORT, BOARDREPORTCreate, BOARDREPORTUpdate
import logging
...
class BOARDREPORTService:
    ...
    async def get_boardreport(self, boardreport_id: int) -> Optional[BOARDREPORT]:
        """Get BOARDREPORT by ID"""
        return await self.db.query(BOARDREPORT).filter(
            and_(
                BOARDREPORT.id == boardreport_id,
    ...
    async def get_boardreports_by_company(
        ...
        """Get all BOARDREPORT forms for a company"""
        return await self.db.query(BOARDREPORT).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/boardreport.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.boardreport import BOARDREPORT, BOARDREPORTCreate
...
async def create_boardreport(
    ...
    """Create new BOARDREPORT form"""
    boardreport_service = BOARDREPORTService(db)
    return await boardreport_service.create_boardreport(
        boardreport_data, 
        current_user.id
    )

@router.get("/{id}", response_model=BOARDREPORTView)
async def get_boardreport(
    ...
    """Get BOARDREPORT by ID"""
    boardreport_service = BOARDREPORTService(db)
    boardreport = await boardreport_service.get_boardreport(id)
    if not boardreport:
        raise HTTPException(status_code=404, detail="BOARDREPORT not found")
    return boardreport
```
...
```python
# File: services/forms/app/models/boardreport.py
...
class BOARDREPORTUpdate(BOARDREPORTBase):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class BOARDREPORTResponse(BOARDREPORTBase):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class BOARDREPORTView(BOARDREPORTResponse):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → BOARDREPORT
   ↓
