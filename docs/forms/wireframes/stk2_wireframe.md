# STK2: Notice of Transfer of Shares

**Form Code:** STK2  
**Form Name:** Notice of Transfer of Shares  
**Regulation:** Companies Act, 2013 - Section 56  
**Filing Timeline:** Within 30 days of transfer  
**Field Count:** 55  
**Status:** ✅ Migrated
...
┌─────────────────────────────────────────────────────────────────────────────┐
│ [Details]  [Parties]  [Shares]  [Attachments]                               │
└─────────────────────────────────────────────────────────────────────────────┘
...
### 4. List All STK2 Forms

```http
GET /api/stk2/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/stk2/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/stk2/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_stk2 (
    ...
    -- Constraints
    CONSTRAINT ck_stk2_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_stk2_company (company_id),
    INDEX idx_stk2_cin (cin),
    INDEX idx_stk2_active (is_active),
    INDEX idx_stk2_created (created_on)
);

-- View for joined data
CREATE VIEW vw_stk2 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/stk2/stk2.component.ts
...
export class STK2Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.stk2Form.valid) {
      this.stk2Service.createSTK2(this.stk2Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/stk2_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.stk2 import STK2, STK2Create, STK2Update
import logging
...
class STK2Service:
    ...
    async def get_stk2(self, stk2_id: int) -> Optional[STK2]:
        """Get STK2 by ID"""
        return await self.db.query(STK2).filter(
            and_(
                STK2.id == stk2_id,
                STK2.is_active == True
            )
    ...
    async def get_stk2s_by_company(
        ...
        """Get all STK2 forms for a company"""
        return await self.db.query(STK2).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/stk2.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.stk2 import STK2, STK2Create
...
async def create_stk2(
    ...
    """Create new STK2 form"""
    stk2_service = STK2Service(db)
    return await stk2_service.create_stk2(
        stk2_data, 
        current_user.id
    )

@router.get("/{id}", response_model=STK2View)
async def get_stk2(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get STK2 by ID"""
    stk2_service = STK2Service(db)
    stk2 = await stk2_service.get_stk2(id)
    if not stk2:
        raise HTTPException(status_code=404, detail="STK2 not found")
    return stk2
```
...
```python
# File: services/forms/app/models/stk2.py
...
class STK2Create(STK2Base):
    pass

class STK2Update(STK2Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class STK2Response(STK2Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class STK2View(STK2Response):
