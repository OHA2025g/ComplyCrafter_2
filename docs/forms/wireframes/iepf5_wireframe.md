...
### 4. List All IEPF5 Forms

```http
GET /api/iepf5/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/iepf5/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/iepf5/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_iepf5 (
    ...
    -- Constraints
    CONSTRAINT ck_iepf5_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_iepf5_company (company_id),
    INDEX idx_iepf5_cin (cin),
    INDEX idx_iepf5_active (is_active),
    INDEX idx_iepf5_created (created_on)
);

-- View for joined data
CREATE VIEW vw_iepf5 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/iepf5/iepf5.component.ts
...
export class IEPF5Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.iepf5Form.valid) {
      this.iepf5Service.createIEPF5(this.iepf5Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/iepf5_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.iepf5 import IEPF5, IEPF5Create, IEPF5Update
import logging
...
class IEPF5Service:
    ...
    async def get_iepf5(self, iepf5_id: int) -> Optional[IEPF5]:
        """Get IEPF5 by ID"""
        return await self.db.query(IEPF5).filter(
            and_(
                IEPF5.id == iepf5_id,
    ...
    async def get_iepf5s_by_company(
        ...
        """Get all IEPF5 forms for a company"""
        return await self.db.query(IEPF5).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/iepf5.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.iepf5 import IEPF5, IEPF5Create
...
async def create_iepf5(
    ...
    """Create new IEPF5 form"""
    iepf5_service = IEPF5Service(db)
    return await iepf5_service.create_iepf5(
        iepf5_data, 
        current_user.id
    )

@router.get("/{id}", response_model=IEPF5View)
async def get_iepf5(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get IEPF5 by ID"""
    iepf5_service = IEPF5Service(db)
    iepf5 = await iepf5_service.get_iepf5(id)
    if not iepf5:
        raise HTTPException(status_code=404, detail="IEPF5 not found")
    return iepf5
```
...
```python
# File: services/forms/app/models/iepf5.py
...
class IEPF5Create(IEPF5Base):
    pass

class IEPF5Update(IEPF5Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class IEPF5Response(IEPF5Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class IEPF5View(IEPF5Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → IEPF5
   ↓
