...
### 4. List All FORM4 Forms

```http
GET /api/form4/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/form4/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/form4/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_form4 (
    ...
    -- Constraints
    CONSTRAINT ck_form4_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_form4_company (company_id),
    INDEX idx_form4_cin (cin),
    INDEX idx_form4_active (is_active),
    INDEX idx_form4_created (created_on)
);

-- View for joined data
CREATE VIEW vw_form4 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/form4/form4.component.ts
...
export class FORM4Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.form4Form.valid) {
      this.form4Service.createFORM4(this.form4Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/form4_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.form4 import FORM4, FORM4Create, FORM4Update
import logging
...
class FORM4Service:
    ...
    async def get_form4(self, form4_id: int) -> Optional[FORM4]:
        """Get FORM4 by ID"""
        return await self.db.query(FORM4).filter(
            and_(
                FORM4.id == form4_id,
    ...
    async def get_form4s_by_company(
        ...
        """Get all FORM4 forms for a company"""
        return await self.db.query(FORM4).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/form4.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.form4 import FORM4, FORM4Create
...
async def create_form4(
    ...
    """Create new FORM4 form"""
    form4_service = FORM4Service(db)
    return await form4_service.create_form4(
        form4_data, 
        current_user.id
    )

@router.get("/{id}", response_model=FORM4View)
async def get_form4(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get FORM4 by ID"""
    form4_service = FORM4Service(db)
    form4 = await form4_service.get_form4(id)
    if not form4:
        raise HTTPException(status_code=404, detail="FORM4 not found")
    return form4
```
...
```python
# File: services/forms/app/models/form4.py
...
class FORM4Create(FORM4Base):
    pass

class FORM4Update(FORM4Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class FORM4Response(FORM4Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class FORM4View(FORM4Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → FORM4
   ↓
