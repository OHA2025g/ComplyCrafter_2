...
### 4. List All SH7 Forms

```http
GET /api/sh7/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/sh7/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/sh7/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_sh7 (
    ...
    -- Constraints
    CONSTRAINT ck_sh7_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_sh7_company (company_id),
    INDEX idx_sh7_cin (cin),
    INDEX idx_sh7_active (is_active),
    INDEX idx_sh7_created (created_on)
);

-- View for joined data
CREATE VIEW vw_sh7 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/sh7/sh7.component.ts
...
export class SH7Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.sh7Form.valid) {
      this.sh7Service.createSH7(this.sh7Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/sh7_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.sh7 import SH7, SH7Create, SH7Update
import logging
...
class SH7Service:
    ...
    async def get_sh7(self, sh7_id: int) -> Optional[SH7]:
        """Get SH7 by ID"""
        return await self.db.query(SH7).filter(
            and_(
                SH7.id == sh7_id,
                SH7.is_active == True
            )
    ...
    async def get_sh7s_by_company(
        ...
        """Get all SH7 forms for a company"""
        return await self.db.query(SH7).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/sh7.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.sh7 import SH7, SH7Create
...
async def create_sh7(
    ...
    return await sh7_service.create_sh7(
        sh7_data, 
        current_user.id
    )

@router.get("/{id}", response_model=SH7View)
async def get_sh7(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get SH7 by ID"""
    sh7_service = SH7Service(db)
    sh7 = await sh7_service.get_sh7(id)
    if not sh7:
        raise HTTPException(status_code=404, detail="SH7 not found")
    return sh7
```
...
```python
# File: services/forms/app/models/sh7.py
...
class SH7Create(SH7Base):
    pass

class SH7Update(SH7Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class SH7Response(SH7Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class SH7View(SH7Response):
