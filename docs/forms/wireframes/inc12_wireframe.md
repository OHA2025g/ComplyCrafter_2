...
### 4. List All INC12 Forms

```http
GET /api/inc12/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/inc12/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/inc12/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_inc12 (
    ...
    -- Constraints
    CONSTRAINT ck_inc12_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_inc12_company (company_id),
    INDEX idx_inc12_cin (cin),
    INDEX idx_inc12_active (is_active),
    INDEX idx_inc12_created (created_on)
);

-- View for joined data
CREATE VIEW vw_inc12 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/inc12/inc12.component.ts
...
export class INC12Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.inc12Form.valid) {
      this.inc12Service.createINC12(this.inc12Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/inc12_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.inc12 import INC12, INC12Create, INC12Update
import logging
...
class INC12Service:
    ...
    async def get_inc12(self, inc12_id: int) -> Optional[INC12]:
        """Get INC12 by ID"""
        return await self.db.query(INC12).filter(
            and_(
                INC12.id == inc12_id,
    ...
    async def get_inc12s_by_company(
        ...
        """Get all INC12 forms for a company"""
        return await self.db.query(INC12).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/inc12.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.inc12 import INC12, INC12Create
...
async def create_inc12(
    ...
    """Create new INC12 form"""
    inc12_service = INC12Service(db)
    return await inc12_service.create_inc12(
        inc12_data, 
        current_user.id
    )

@router.get("/{id}", response_model=INC12View)
async def get_inc12(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get INC12 by ID"""
    inc12_service = INC12Service(db)
    inc12 = await inc12_service.get_inc12(id)
    if not inc12:
        raise HTTPException(status_code=404, detail="INC12 not found")
    return inc12
```
...
```python
# File: services/forms/app/models/inc12.py
...
class INC12Create(INC12Base):
    pass

class INC12Update(INC12Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class INC12Response(INC12Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class INC12View(INC12Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → INC12
   ↓
