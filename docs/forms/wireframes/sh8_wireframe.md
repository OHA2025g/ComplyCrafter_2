...
### 4. List All SH8 Forms

```http
GET /api/sh8/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/sh8/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/sh8/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_sh8 (
    ...
    -- Constraints
    CONSTRAINT ck_sh8_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_sh8_company (company_id),
    INDEX idx_sh8_cin (cin),
    INDEX idx_sh8_active (is_active),
    INDEX idx_sh8_created (created_on)
);

-- View for joined data
CREATE VIEW vw_sh8 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/sh8/sh8.component.ts
...
export class SH8Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.sh8Form.valid) {
      this.sh8Service.createSH8(this.sh8Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/sh8_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.sh8 import SH8, SH8Create, SH8Update
import logging
...
class SH8Service:
    ...
    async def get_sh8(self, sh8_id: int) -> Optional[SH8]:
        """Get SH8 by ID"""
        return await self.db.query(SH8).filter(
            and_(
                SH8.id == sh8_id,
                SH8.is_active == True
            )
    ...
    async def get_sh8s_by_company(
        ...
        """Get all SH8 forms for a company"""
        return await self.db.query(SH8).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/sh8.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.sh8 import SH8, SH8Create
...
async def create_sh8(
    ...
    return await sh8_service.create_sh8(
        sh8_data, 
        current_user.id
    )

@router.get("/{id}", response_model=SH8View)
async def get_sh8(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get SH8 by ID"""
    sh8_service = SH8Service(db)
    sh8 = await sh8_service.get_sh8(id)
    if not sh8:
        raise HTTPException(status_code=404, detail="SH8 not found")
    return sh8
```
...
```python
# File: services/forms/app/models/sh8.py
...
class SH8Create(SH8Base):
    pass

class SH8Update(SH8Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class SH8Response(SH8Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class SH8View(SH8Response):
