...
### 4. List All CHG4 Forms

```http
GET /api/chg4/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/chg4/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/chg4/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_chg4 (
    ...
    -- Constraints
    CONSTRAINT ck_chg4_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_chg4_company (company_id),
    INDEX idx_chg4_cin (cin),
    INDEX idx_chg4_active (is_active),
    INDEX idx_chg4_created (created_on)
);

-- View for joined data
CREATE VIEW vw_chg4 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/chg4/chg4.component.ts
...
export class CHG4Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.chg4Form.valid) {
      this.chg4Service.createCHG4(this.chg4Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/chg4_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.chg4 import CHG4, CHG4Create, CHG4Update
import logging
...
class CHG4Service:
    ...
    async def get_chg4(self, chg4_id: int) -> Optional[CHG4]:
        """Get CHG4 by ID"""
        return await self.db.query(CHG4).filter(
            and_(
                CHG4.id == chg4_id,
                CHG4.is_active == True
            )
    ...
    async def get_chg4s_by_company(
        ...
        """Get all CHG4 forms for a company"""
        return await self.db.query(CHG4).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/chg4.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.chg4 import CHG4, CHG4Create
...
async def create_chg4(
    ...
    """Create new CHG4 form"""
    chg4_service = CHG4Service(db)
    return await chg4_service.create_chg4(
        chg4_data, 
        current_user.id
    )

@router.get("/{id}", response_model=CHG4View)
async def get_chg4(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get CHG4 by ID"""
    chg4_service = CHG4Service(db)
    chg4 = await chg4_service.get_chg4(id)
    if not chg4:
        raise HTTPException(status_code=404, detail="CHG4 not found")
    return chg4
```
...
```python
# File: services/forms/app/models/chg4.py
...
class CHG4Create(CHG4Base):
    pass

class CHG4Update(CHG4Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class CHG4Response(CHG4Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class CHG4View(CHG4Response):
