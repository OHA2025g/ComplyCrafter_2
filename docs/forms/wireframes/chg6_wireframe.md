...
### 4. List All CHG6 Forms

```http
GET /api/chg6/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/chg6/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/chg6/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_chg6 (
    ...
    -- Constraints
    CONSTRAINT ck_chg6_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_chg6_company (company_id),
    INDEX idx_chg6_cin (cin),
    INDEX idx_chg6_active (is_active),
    INDEX idx_chg6_created (created_on)
);

-- View for joined data
CREATE VIEW vw_chg6 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/chg6/chg6.component.ts
...
export class CHG6Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.chg6Form.valid) {
      this.chg6Service.createCHG6(this.chg6Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/chg6_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.chg6 import CHG6, CHG6Create, CHG6Update
import logging
...
class CHG6Service:
    ...
    async def get_chg6(self, chg6_id: int) -> Optional[CHG6]:
        """Get CHG6 by ID"""
        return await self.db.query(CHG6).filter(
            and_(
                CHG6.id == chg6_id,
                CHG6.is_active == True
            )
    ...
    async def get_chg6s_by_company(
        ...
        """Get all CHG6 forms for a company"""
        return await self.db.query(CHG6).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/chg6.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.chg6 import CHG6, CHG6Create
...
async def create_chg6(
    ...
    """Create new CHG6 form"""
    chg6_service = CHG6Service(db)
    return await chg6_service.create_chg6(
        chg6_data, 
        current_user.id
    )

@router.get("/{id}", response_model=CHG6View)
async def get_chg6(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get CHG6 by ID"""
    chg6_service = CHG6Service(db)
    chg6 = await chg6_service.get_chg6(id)
    if not chg6:
        raise HTTPException(status_code=404, detail="CHG6 not found")
    return chg6
```
...
```python
# File: services/forms/app/models/chg6.py
...
class CHG6Create(CHG6Base):
    pass

class CHG6Update(CHG6Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class CHG6Response(CHG6Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class CHG6View(CHG6Response):
