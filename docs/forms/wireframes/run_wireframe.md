# RUN: Annual Return of a Company

**Form Code:** RUN  
**Form Name:** Annual Return of a Company  
**Regulation:** Companies Act, 2013 - Section 92  
**Filing Timeline:** Within 60 days of AGM  
**Field Count:** 200  
**Status:** ✅ Migrated
...
┌─────────────────────────────────────────────────────────────────────────────┐
│ [Company]  [Capital]  [Members]  [Management]  [Attachments]                │
└─────────────────────────────────────────────────────────────────────────────┘
...
### 4. List All RUN Forms

```http
GET /api/run/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/run/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/run/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_run (
    ...
    -- Constraints
    CONSTRAINT ck_run_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_run_company (company_id),
    INDEX idx_run_cin (cin),
    INDEX idx_run_active (is_active),
    INDEX idx_run_created (created_on)
);

-- View for joined data
CREATE VIEW vw_run AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/run/run.component.ts
...
export class RUNComponent implements OnInit {
  ...
  onSubmit(): void {
    if (this.runForm.valid) {
      this.runService.createRUN(this.runForm.value)
        .subscribe({
...
```python
# File: services/forms/app/services/run_service.py
...
class RUNService:
    ...
    async def get_run(self, run_id: int) -> Optional[RUN]:
        """Get RUN by ID"""
        return await self.db.query(RUN).filter(
            and_(
                RUN.id == run_id,
                RUN.is_active == True
            )
    ...
    async def get_runs_by_company(
        ...
        """Get all RUN forms for a company"""
        return await self.db.query(RUN).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/run.py
...
@router.get("/{id}", response_model=RUNView)
async def get_run(
    id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
    ...
    """Get RUN by ID"""
    run_service = RUNService(db)
    run = await run_service.get_run(id)
    if not run:
...
```python
# File: services/forms/app/models/run.py
