...
### 4. List All FORM11 Forms

```http
GET /api/form11/?skip=0&limit=100
Authorization: Bearer {token}
...
```http
GET /api/form11/company/{company_id}
Authorization: Bearer {token}
...
```http
PATCH /api/form11/{id}/status/{status}
Authorization: Bearer {token}
...
CREATE TABLE tbl_form11 (
    ...
    -- Constraints
    CONSTRAINT ck_form11_cin_length CHECK (LENGTH(cin) = 21),
    
    -- Indexes
    INDEX idx_form11_company (company_id),
    INDEX idx_form11_cin (cin),
    INDEX idx_form11_active (is_active),
    INDEX idx_form11_created (created_on)
);

-- View for joined data
CREATE VIEW vw_form11 AS
SELECT 
    f.*,
...
```typescript
// File: frontend/apps/portal/src/app/forms/form11/form11.component.ts
...
export class FORM11Component implements OnInit {
  ...
  onSubmit(): void {
    if (this.form11Form.valid) {
      this.form11Service.createFORM11(this.form11Form.value)
        .subscribe({
...
```python
# File: services/forms/app/services/form11_service.py

from sqlalchemy.orm import Session
from sqlalchemy import and_
from typing import List, Optional
from ..models.form11 import FORM11, FORM11Create, FORM11Update
import logging
...
class FORM11Service:
    ...
    async def get_form11(self, form11_id: int) -> Optional[FORM11]:
        """Get FORM11 by ID"""
        return await self.db.query(FORM11).filter(
            and_(
                FORM11.id == form11_id,
    ...
    async def get_form11s_by_company(
        ...
        """Get all FORM11 forms for a company"""
        return await self.db.query(FORM11).filter(
            and_(
...
```python
# File: services/forms/app/api/routes/form11.py

from fastapi import APIRouter, Depends, HTTPException, status
from sqlalchemy.orm import Session
from typing import List
from ..dependencies import get_current_user, get_db
from ...models.form11 import FORM11, FORM11Create
...
async def create_form11(
    ...
    """Create new FORM11 form"""
    form11_service = FORM11Service(db)
    return await form11_service.create_form11(
        form11_data, 
        current_user.id
    )

@router.get("/{id}", response_model=FORM11View)
async def get_form11(
    ...
    """Get FORM11 by ID"""
    form11_service = FORM11Service(db)
    form11 = await form11_service.get_form11(id)
    if not form11:
        raise HTTPException(status_code=404, detail="FORM11 not found")
    return form11
```
...
```python
# File: services/forms/app/models/form11.py
...
class FORM11Create(FORM11Base):
    pass

class FORM11Update(FORM11Base):
    company_id: Optional[int] = None
    cin: Optional[str] = None
    # All fields optional for partial updates

class FORM11Response(FORM11Base):
    id: int
    created_by: int
    created_on: datetime
    is_active: bool

class FORM11View(FORM11Response):
...
Step 1: Login to ComplyCrafter
   ↓
Step 2: Navigate to Forms → FORM11
   ↓
