...
async def create_mgt8(
    ...
    try:
        ...
        mgt8 = await mgt8_service.create_mgt8(mgt8_data, current_user.id)
        return mgt8
...
async def get_mgt8s(
    ...
    try:
        mgt8_service = MGT8Service(db)
        mgt8s = await mgt8_service.get_mgt8s(skip=skip, limit=limit)
        return mgt8s
    except Exception as e:
...
async def get_mgt8(
    ...
    try:
        mgt8_service = MGT8Service(db)
        mgt8 = await mgt8_service.get_mgt8(mgt8_id)
        if not mgt8:
            raise HTTPException(
...
async def update_mgt8(
    ...
    try:
        mgt8_service = MGT8Service(db)
        mgt8 = await mgt8_service.update_mgt8(mgt8_id, mgt8_data, current_user.id)
        if not mgt8:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="MGT8 not found"
...
async def delete_mgt8(
    ...
    try:
        mgt8_service = MGT8Service(db)
        success = await mgt8_service.delete_mgt8(mgt8_id, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="MGT8 not found"
...
async def get_mgt8s_by_company(
    ...
    try:
        mgt8_service = MGT8Service(db)
        mgt8s = await mgt8_service.get_mgt8s_by_company(company_id)
        return mgt8s
    except Exception as e:
...
async def change_mgt8_status(
    ...
    try:
        mgt8_service = MGT8Service(db)
        success = await mgt8_service.change_status(mgt8_id, status, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="MGT8 not found"
