...
async def create_inc4(
    ...
    try:
        ...
        inc4 = await inc4_service.create_inc4(inc4_data, current_user.id)
        return inc4
...
async def get_inc4s(
    ...
    try:
        inc4_service = INC4Service(db)
        inc4s = await inc4_service.get_inc4s(skip=skip, limit=limit)
        return inc4s
    except Exception as e:
...
async def get_inc4(
    ...
    try:
        inc4_service = INC4Service(db)
        inc4 = await inc4_service.get_inc4(inc4_id)
        if not inc4:
            raise HTTPException(
...
async def update_inc4(
    ...
    try:
        inc4_service = INC4Service(db)
        inc4 = await inc4_service.update_inc4(inc4_id, inc4_data, current_user.id)
        if not inc4:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="INC4 not found"
...
async def delete_inc4(
    ...
    try:
        inc4_service = INC4Service(db)
        success = await inc4_service.delete_inc4(inc4_id, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="INC4 not found"
...
async def get_inc4s_by_company(
    ...
    try:
        inc4_service = INC4Service(db)
        inc4s = await inc4_service.get_inc4s_by_company(company_id)
        return inc4s
    except Exception as e:
...
async def change_inc4_status(
    ...
    try:
        inc4_service = INC4Service(db)
        success = await inc4_service.change_status(inc4_id, status, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="INC4 not found"
