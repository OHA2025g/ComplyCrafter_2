...
async def create_chg1(
    ...
    try:
        ...
        chg1 = chg1_service.create_chg1(chg1_data, current_user.id)
        return chg1
...
async def get_chg1s(
    ...
    try:
        chg1_service = CHG1Service(db)
        chg1s = chg1_service.get_chg1s(skip=skip, limit=limit)
        return chg1s
    except Exception as e:
...
async def get_chg1(
    ...
    try:
        chg1_service = CHG1Service(db)
        chg1 = chg1_service.get_chg1(chg1_id)
        if not chg1:
            raise HTTPException(
...
async def update_chg1(
    ...
    try:
        chg1_service = CHG1Service(db)
        chg1 = chg1_service.update_chg1(chg1_id, chg1_data, current_user.id)
        if not chg1:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="CHG1 not found"
...
async def delete_chg1(
    ...
    try:
        chg1_service = CHG1Service(db)
        success = chg1_service.delete_chg1(chg1_id, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="CHG1 not found"
...
async def get_chg1s_by_company(
    ...
    try:
        chg1_service = CHG1Service(db)
        chg1s = chg1_service.get_chg1s_by_company(company_id)
        return chg1s
    except Exception as e:
...
async def change_chg1_status(
    ...
    try:
        chg1_service = CHG1Service(db)
        success = chg1_service.change_status(chg1_id, status, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="CHG1 not found"
