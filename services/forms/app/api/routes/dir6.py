...
async def create_dir6(
    ...
    try:
        ...
        dir6 = await dir6_service.create_dir6(dir6_data, current_user.id)
        return dir6
...
async def get_dir6s(
    ...
    try:
        dir6_service = DIR6Service(db)
        dir6s = await dir6_service.get_dir6s(skip=skip, limit=limit)
        return dir6s
    except Exception as e:
...
async def get_dir6(
    ...
    try:
        dir6_service = DIR6Service(db)
        dir6 = await dir6_service.get_dir6(dir6_id)
        if not dir6:
            raise HTTPException(
...
async def update_dir6(
    ...
    try:
        dir6_service = DIR6Service(db)
        dir6 = await dir6_service.update_dir6(dir6_id, dir6_data, current_user.id)
        if not dir6:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="DIR6 not found"
...
async def delete_dir6(
    ...
    try:
        dir6_service = DIR6Service(db)
        success = await dir6_service.delete_dir6(dir6_id, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="DIR6 not found"
...
async def get_dir6s_by_company(
    ...
    try:
        dir6_service = DIR6Service(db)
        dir6s = await dir6_service.get_dir6s_by_company(company_id)
        return dir6s
    except Exception as e:
...
async def change_dir6_status(
    ...
    try:
        dir6_service = DIR6Service(db)
        success = await dir6_service.change_status(dir6_id, status, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="DIR6 not found"
