...
async def create_run(
    ...
    try:
        ...
        run = await run_service.create_run(run_data, current_user.id)
        return run
...
async def update_run(
    ...
    try:
        run_service = RUNService(db)
        run = await run_service.update_run(run_id, run_data, current_user.id)
        if not run:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="RUN not found"
...
async def delete_run(
    ...
    try:
        run_service = RUNService(db)
        success = await run_service.delete_run(run_id, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="RUN not found"
...
async def get_runs_by_company(
    ...
    try:
        run_service = RUNService(db)
        runs = await run_service.get_runs_by_company(company_id)
        return runs
    except Exception as e:
...
async def change_run_status(
    ...
    try:
        run_service = RUNService(db)
        success = await run_service.change_status(run_id, status, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="RUN not found"
