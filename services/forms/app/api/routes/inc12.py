...
async def create_inc12(
    ...
    try:
        ...
        inc12 = await inc12_service.create_inc12(inc12_data, current_user.id)
        return inc12
...
async def get_inc12s(
    ...
    try:
        inc12_service = INC12Service(db)
        inc12s = await inc12_service.get_inc12s(skip=skip, limit=limit)
        return inc12s
    except Exception as e:
...
async def get_inc12(
    ...
    try:
        inc12_service = INC12Service(db)
        inc12 = await inc12_service.get_inc12(inc12_id)
        if not inc12:
            raise HTTPException(
...
async def update_inc12(
    ...
    try:
        inc12_service = INC12Service(db)
        inc12 = await inc12_service.update_inc12(inc12_id, inc12_data, current_user.id)
        if not inc12:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="INC12 not found"
...
async def delete_inc12(
    ...
    try:
        inc12_service = INC12Service(db)
        success = await inc12_service.delete_inc12(inc12_id, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="INC12 not found"
...
async def get_inc12s_by_company(
    ...
    try:
        inc12_service = INC12Service(db)
        inc12s = await inc12_service.get_inc12s_by_company(company_id)
        return inc12s
    except Exception as e:
...
async def change_inc12_status(
    ...
    try:
        inc12_service = INC12Service(db)
        success = await inc12_service.change_status(inc12_id, status, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="INC12 not found"
