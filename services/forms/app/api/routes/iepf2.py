...
async def create_iepf2(
    ...
    try:
        ...
        iepf2 = await iepf2_service.create_iepf2(iepf2_data, current_user.id)
        return iepf2
...
async def get_iepf2s(
    ...
    try:
        iepf2_service = IEPF2Service(db)
        iepf2s = await iepf2_service.get_iepf2s(skip=skip, limit=limit)
        return iepf2s
    except Exception as e:
...
async def get_iepf2(
    ...
    try:
        iepf2_service = IEPF2Service(db)
        iepf2 = await iepf2_service.get_iepf2(iepf2_id)
        if not iepf2:
            raise HTTPException(
...
async def update_iepf2(
    ...
    try:
        iepf2_service = IEPF2Service(db)
        iepf2 = await iepf2_service.update_iepf2(iepf2_id, iepf2_data, current_user.id)
        if not iepf2:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="IEPF2 not found"
...
async def delete_iepf2(
    ...
    try:
        iepf2_service = IEPF2Service(db)
        success = await iepf2_service.delete_iepf2(iepf2_id, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="IEPF2 not found"
...
async def get_iepf2s_by_company(
    ...
    try:
        iepf2_service = IEPF2Service(db)
        iepf2s = await iepf2_service.get_iepf2s_by_company(company_id)
        return iepf2s
    except Exception as e:
...
async def change_iepf2_status(
    ...
    try:
        iepf2_service = IEPF2Service(db)
        success = await iepf2_service.change_status(iepf2_id, status, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="IEPF2 not found"
