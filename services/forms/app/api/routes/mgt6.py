...
async def create_mgt6(
    ...
    try:
        ...
        mgt6 = mgt6_service.create_mgt6(mgt6_data, current_user.id)
        return mgt6
...
async def get_mgt6s(
    ...
    try:
        mgt6_service = MGT6Service(db)
        mgt6s = mgt6_service.get_mgt6s(skip=skip, limit=limit)
        return mgt6s
    except Exception as e:
...
async def get_mgt6(
    ...
    try:
        mgt6_service = MGT6Service(db)
        mgt6 = mgt6_service.get_mgt6(mgt6_id)
        if not mgt6:
            raise HTTPException(
...
async def update_mgt6(
    ...
    try:
        mgt6_service = MGT6Service(db)
        mgt6 = mgt6_service.update_mgt6(mgt6_id, mgt6_data, current_user.id)
        if not mgt6:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="MGT6 not found"
...
async def delete_mgt6(
    ...
    try:
        mgt6_service = MGT6Service(db)
        success = mgt6_service.delete_mgt6(mgt6_id, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="MGT6 not found"
...
async def get_mgt6s_by_company(
    ...
    try:
        mgt6_service = MGT6Service(db)
        mgt6s = mgt6_service.get_mgt6s_by_company(company_id)
        return mgt6s
    except Exception as e:
...
async def change_mgt6_status(
    ...
    try:
        mgt6_service = MGT6Service(db)
        success = mgt6_service.change_status(mgt6_id, status, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="MGT6 not found"
