...
async def create_stk2(
    ...
    try:
        ...
        stk2 = stk2_service.create_stk2(stk2_data, current_user.id)
        return stk2
...
async def get_stk2s(
    ...
    try:
        stk2_service = STK2Service(db)
        stk2s = stk2_service.get_stk2s(skip=skip, limit=limit)
        return stk2s
    except Exception as e:
...
async def get_stk2(
    ...
    try:
        stk2_service = STK2Service(db)
        stk2 = stk2_service.get_stk2(stk2_id)
        if not stk2:
            raise HTTPException(
...
async def update_stk2(
    ...
    try:
        stk2_service = STK2Service(db)
        stk2 = stk2_service.update_stk2(stk2_id, stk2_data, current_user.id)
        if not stk2:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="STK2 not found"
...
async def delete_stk2(
    ...
    try:
        stk2_service = STK2Service(db)
        success = stk2_service.delete_stk2(stk2_id, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="STK2 not found"
...
async def get_stk2s_by_company(
    ...
    try:
        stk2_service = STK2Service(db)
        stk2s = stk2_service.get_stk2s_by_company(company_id)
        return stk2s
    except Exception as e:
...
async def change_stk2_status(
    ...
    try:
        stk2_service = STK2Service(db)
        success = stk2_service.change_status(stk2_id, status, current_user.id)
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail="STK2 not found"
