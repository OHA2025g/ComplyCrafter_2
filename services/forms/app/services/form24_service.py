...
class Form24Service:
    ...
    async def create_form24(self, form24_data: Form24Create, user_id: int) -> Form24:
    ...
    async def get_form24s(self, skip: int = 0, limit: int = 100) -> List[Form24]:
        """Get all active form24s with pagination"""
        try:
            result = await self.db.execute(
                select(Form24).where(Form24.is_active == True).offset(skip).limit(limit)
            )
    ...
    async def get_form24s_by_company(self, company_id: int) -> List[Form24]:
    ...
    async def update_form24(self, form24_id: int, form24_data: Form24Update, user_id: int) -> Optional[Form24]:
        """Update a form24 record"""
        try:
            form24 = await self.get_form24(form24_id)
            if not form24:
                return None
            ...
            form24.updated_on = datetime.utcnow()
            await self.db.commit()
            await self.db.refresh(form24)
    ...
    async def delete_form24(self, form24_id: int, user_id: int) -> bool:
    ...
    async def change_status(self, form24_id: int, status: bool, user_id: int) -> bool:
        """Change the active status of a form24"""
        try:
            result = await self.db.execute(
                select(Form24).where(Form24.id == form24_id)
            )
            form24 = result.scalar_one_or_none()
            if not form24:
                return False
