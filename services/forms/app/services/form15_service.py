...
class Form15Service:
    ...
    async def create_form15(self, form15_data: Form15Create, user_id: int) -> Form15:
    ...
    async def get_form15s(self, skip: int = 0, limit: int = 100) -> List[Form15]:
        """Get all active form15s with pagination"""
        try:
            result = await self.db.execute(
                select(Form15).where(Form15.is_active == True).offset(skip).limit(limit)
            )
    ...
    async def get_form15s_by_company(self, company_id: int) -> List[Form15]:
    ...
    async def update_form15(self, form15_id: int, form15_data: Form15Update, user_id: int) -> Optional[Form15]:
        """Update a form15 record"""
        try:
            form15 = await self.get_form15(form15_id)
            if not form15:
                return None
            ...
            form15.updated_on = datetime.utcnow()
            await self.db.commit()
            await self.db.refresh(form15)
    ...
    async def delete_form15(self, form15_id: int, user_id: int) -> bool:
    ...
    async def change_status(self, form15_id: int, status: bool, user_id: int) -> bool:
        """Change the active status of a form15"""
        try:
            result = await self.db.execute(
                select(Form15).where(Form15.id == form15_id)
            )
            form15 = result.scalar_one_or_none()
            if not form15:
                return False
