...
class Form22Service:
    ...
    async def create_form22(self, form22_data: Form22Create, user_id: int) -> Form22:
    ...
    async def get_form22s(self, skip: int = 0, limit: int = 100) -> List[Form22]:
        """Get all active form22s with pagination"""
        try:
            result = await self.db.execute(
                select(Form22).where(Form22.is_active == True).offset(skip).limit(limit)
            )
    ...
    async def get_form22s_by_company(self, company_id: int) -> List[Form22]:
    ...
    async def update_form22(self, form22_id: int, form22_data: Form22Update, user_id: int) -> Optional[Form22]:
        """Update a form22 record"""
        try:
            form22 = await self.get_form22(form22_id)
            if not form22:
                return None
            ...
            form22.updated_on = datetime.utcnow()
            await self.db.commit()
            await self.db.refresh(form22)
    ...
    async def delete_form22(self, form22_id: int, user_id: int) -> bool:
    ...
    async def change_status(self, form22_id: int, status: bool, user_id: int) -> bool:
        """Change the active status of a form22"""
        try:
            result = await self.db.execute(
                select(Form22).where(Form22.id == form22_id)
            )
            form22 = result.scalar_one_or_none()
            if not form22:
                return False
